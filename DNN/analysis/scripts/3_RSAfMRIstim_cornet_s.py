...
occluders = ['unoccluded','barHorz08']#'naturalTextured2','barHorz08']
occludersNoLevels = ['unoccluded','naturalTextured2']
...
for l, layer in enumerate(layers):
    ...
    for trainset in trainsets:
        ...
        for timesString in ['2_2_4_2', '5_5_10_5']:
            ...
            outDir = f'DNN/analysis/results/{model}/{timesString}/{trainset}/RSAfMRIstim/{version}/RSMs'
            os.makedirs(outDir, exist_ok=True)

            for occluder in occluders:
                if occluder in occludersNoLevels:
                    coverageString = None
                    modelDir = os.path.join('DNN/data', f'{model}_{timesString}', trainset, 'fromPretrained', occluder)
                else:
                    coverage = .5
                    modelDir = os.path.join('DNN/data', f'{model}_{timesString}', trainset, 'fromPretrained', occluder, str(int(coverage * 100)))
                ...
                for c, condName in enumerate(condNames):
                    ...
                    print(f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} | Collating responses... |'
                          f' Model: {model}_{timesString} | Layer: {layer} | Trained on: {trainset} |'
            ...
            plt.tight_layout()
            outDir = f'DNN/analysis/results/{model}/{timesString}/{trainset}/RSAfMRIstim/{version}/occluded'
            os.makedirs(outDir, exist_ok=True)
            ...
            plt.legend(title='', bbox_to_anchor=(1.04, 1), loc="upper left", frameon=False)
            plt.tight_layout()
            outDir = f'DNN/analysis/results/{model}/{timesString}/{trainset}/RSAfMRIstim/{version}/occVunocc'
            os.makedirs(outDir, exist_ok=True)
            fig.savefig(os.path.join(outDir, f'{layer}.png'))
            plt.show()
            plt.close()
            ...
            plt.title(f'layer: {layer}')
            plt.tight_layout()
            outDir = f'DNN/analysis/results/{model}/{timesString}/{trainset}/RSAfMRIstim/{version}/regression'
            os.makedirs(outDir, exist_ok=True)
            plt.savefig(os.path.join(outDir, f'{layer}.png'))
            plt.show()
            plt.close()

    RSMpath = f'DNN/analysis/results/{model}/{timesString}/{trainset}/RSAfMRIstim/{version}/RSMs.pkl'
