...
templateResponsePath = f'DNN/data/{model}/{trainsets[0]}/fromPretrained/unoccluded/responses/fMRIstim/{version}/bear_none.pkl'
    ...
    for trainset in trainsets:
        ...
        for occluder in occluders:
            if occluder in occludersNoLevels:
                coverageString = None
                modelDir = os.path.join('DNN/data', model, trainset, 'fromPretrained', occluder)
            else:
                coverage = .5
                modelDir = os.path.join('DNN/data', model, trainset, 'fromPretrained', occluder, str(int(coverage * 100)))
            ...
            for c, condName in enumerate(condNames):
                ...
                allResponses.append(np.array(torch.Tensor.cpu(response[l+1].flatten())))
