...
layers2use = 'relu'
exemplarNames = [os.path.basename(x)[:-15] for x in sorted(glob.glob('DNN/images/withinCategoryBarHorz04/*unoccluded*'))]
occPositions = ['unoccluded','upper','lower']
...
for model in models:
    ...
    for l, layer in theseLayers:
        ...
        for dataset in datasets:
            ...
            outDir = f'DNN/analysis/results/{model}/{dataset}/RSAwithinCategory/RSMs'
            ...
            for occluder in occluders:
                ...
                allResponses = []
                responseDir = os.path.join(modelDir, f'responses/withinCategory')
                ...
                for cA, condNameA in enumerate(condNames):
                    ...
                    exemplarA = condNameA[:-len(occluderA)]

                    for cB, condNameB in enumerate(condNames):

                        occluderB = condNameB.split('_')[-1]
                        exemplarB = condNameB[:-len(occluderB)]

                        r = matrix[cA, cB]

                        # 2x2 exemplar by occluder position (occluded images only)
                        if occluderA != 'unoccluded' and occluderB != 'unoccluded':
                        ...
                        # 2x2, exemplar by one occluded yes/no
                        elif occluderA == 'unoccluded' and occluderB == 'unoccluded':  # neither occluded
                            if exemplarA == exemplarB:
                                sameExemBothUnocc.append(r)
                            else:
                                diffExemBothUnocc.append(r)
                        elif int(occluderA == 'unoccluded') + int(occluderB == 'unoccluded') == 1:  # one occluded
                            if exemplarA == exemplarB:
            ...
            plt.tight_layout()
            outDir = f'DNN/analysis/results/{model}/{dataset}/RSAwithinCategory/occluded'
            os.makedirs(outDir, exist_ok=True)
            ...
            plt.legend(title='', bbox_to_anchor=(1.04, 1), loc="upper left", frameon=False)
            plt.tight_layout()
            outDir = f'DNN/analysis/results/{model}/{dataset}/RSAwithinCategory/occVunocc'
            os.makedirs(outDir, exist_ok=True)
            fig.savefig(os.path.join(outDir, f'{layer}.png'))
            plt.show()
            plt.close()
            ...
            plt.title(f'layer: {layer}')
            plt.tight_layout()
            outDir = f'DNN/analysis/results/{model}/{dataset}/RSAwithinCategory/regression'
            os.makedirs(outDir, exist_ok=True)
            plt.savefig(os.path.join(outDir, f'{layer}.png'))
            plt.show()
            plt.close()

RSMpath = f'DNN/analysis/results/{model}/{dataset}/RSAwithinCategory/RSMs.pkl'
